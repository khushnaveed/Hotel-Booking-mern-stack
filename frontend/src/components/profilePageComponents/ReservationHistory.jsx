...
export default function ReservationHistory() {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const [roomRes, eventRes] = await Promise.all([
          axios.get("/order/my-orders", { headers: { token } }),
          axios.get("/order-events/my-orders", { headers: { token } }), // <-- fixed path
        ]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      {roomOrders.length > 0 && (
        <>
          <h2 className="text-2xl font-bold text-[#8E7037] mb-6">
            Room Reservations
          </h2>
          {roomOrders.map((order) => (
            <div
              ...
              <div className="mb-4">
                <h2 className="text-lg font-semibold text-[#8E7037]">
              ...
              <h3 className="text-md font-medium text-[#8E7037] border-b border-gray-200 pb-2 mb-4">
                Booked Room(s)
              </h3>
        ...
        <>
          ...
          {eventOrders.map((order) => (
            <div
              key={order._id}
              className="bg-white shadow-md p-6 mb-10 border border-gray-100">
              <h3 className="text-lg font-semibold text-[#8E7037] mb-2">
                Order ID: {order._id}
              </h3>
